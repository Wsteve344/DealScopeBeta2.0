/*
  # Sample Deal Data Migration
  
  1. Creates a sample deal with address '3819 GREENRIDGE dr'
  2. Adds comprehensive section data including:
    ...
    - Financing details
    - Market analysis
  3. Handles audit logging by temporarily disabling triggers
*/
...
DECLARE
  v_deal_id uuid;
BEGIN
  INSERT INTO deals (address, investor_id, status, progress)
  SELECT 
    '3819 GREENRIDGE dr',
  ...
  -- Get deal ID if insert didn't happen
  IF v_deal_id IS NULL THEN
    SELECT id INTO v_deal_id FROM deals WHERE address = '3819 GREENRIDGE dr';
  END IF;
